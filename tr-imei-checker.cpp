...
void curlse() {
	...
	for (int i = 0 ; i < nmb_; i++) {
		...
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());  //c_str olmas� �nemli...
...
int main(int argc, const char* args[]) {
	...
	cout << "Proxy listenizdeki proxyler �ifreli mi? \n1. Evet\n2. Hay�r\n>> ";
	cin >> secim;
	if (secim == 1)
		sifreliproxy = true;
	else if (secim == 2)
	...
	//proxy kullan�rken proxy responde code i�in: CURLINFO_HTTP_CONNECTCODE
	...
	if (curl) {
		...
		curl_easy_setopt(curl1, CURLOPT_SSL_VERIFYPEER, 1);

		curl_easy_setopt(curl1, CURLOPT_SSL_VERIFYHOST, 2);
		...
		curl_easy_setopt(curl1, CURLOPT_POSTFIELDS, post.c_str());  //c_str olmas� �nemli...
		...
		std::cout << "Kaynak: " << kaynaklar[1] << std::endl;
		std::cout << "Sorgu Tarihi: " << kaynaklar[2] << std::endl;
		std::cout << "Marka/Model: " << kaynaklar[3] << std::endl<<endl<<endl;   //proxy laz�m
